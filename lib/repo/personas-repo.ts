...
import type { Persona, PersonasDoc, Region } from "@/lib/types/messaging";

const cache: { doc: PersonasDoc | null } = { doc: null };
...
export function listPersonas(): Pick<
  Persona,
  "id" | "class" | "label" | "regions"
>[] {
  return loadDoc().personas.map((p) => ({
    id: p.id,
    class: p.class,
    label: p.label,
    regions: p.regions,
