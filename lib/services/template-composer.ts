import { getRegion } from "../repo/personas-repo";
import type {
  ComposeRequest,
  ComposeResponse,
  RegionSpec,
...
function hasURLInBody(text: string) {
...
export function compose(req: ComposeRequest): ComposeResponse {
  ...
  if (req.channel === "whatsapp") {
    ...
    const headerR = header
      ? render(header, req.variables)
      : { rendered: "", used: [] };
    const bodyR = render(ensureOptOut(w.body, req.marketing), req.variables);
    const footerR = w.footer
      ? render(ensureOptOut(w.footer, req.marketing), req.variables)
      : { rendered: "", used: [] };
    raw = { header: w.header, body: w.body, footer: w.footer, cta: w.cta };
    rendered = {
      header: headerR.rendered,
      body: bodyR.rendered,
      footer: footerR.rendered,
    };
    placeholdersUsed = [
      ...new Set([...headerR.used, ...bodyR.used, ...footerR.used]),
    ];
    if (rendered.body.length > WA_BODY_MAX)
      errors.push(`WhatsApp body > ${WA_BODY_MAX} chars`);
    if (header && header.length > WA_HEADER_MAX)
      errors.push(`WhatsApp header > ${WA_HEADER_MAX} chars`);
    if (hasURLInBody(w.body) && !w.cta?.url_template)
      errors.push("WhatsApp: link no body sem CTA URL");
  }
  ...
  if (req.channel === "email") {
    ...
    rendered = {
      subject:
        subj.length > EMAIL_SUBJ_MAX
          ? subj.slice(0, EMAIL_SUBJ_MAX - 1) + "…"
          : subj,
      preheader:
        pre.length > EMAIL_PRE_MAX
          ? pre.slice(0, EMAIL_PRE_MAX - 1) + "…"
          : pre,
    };
  ...
  if (req.channel === "telegram") {
    ...
    if ((t.keyboard?.length ?? 0) > 4)
      errors.push("Telegram: >4 botões na mesma linha");
  }
