...
describe("VizDataTable Component", () => {
  ...
  it("applies bordered style when bordered prop is true", () => {
    const { getByTestId } = render(<VizDataTable data={sampleData} bordered />);
  ...
  it("applies compact style when compact prop is true", () => {
    const { getByTestId } = render(<VizDataTable data={sampleData} compact />);
  ...
  it("renders caption when caption prop is provided", () => {
    ...
    const { getByText } = render(
      <VizDataTable data={sampleData} caption={captionText} />
    );
  ...
  it("handles complex object values by converting them to JSON strings", () => {
    ...
    expect(addressCell?.textContent).toContain('{"street":"Rua A","city":"São Paulo"}');
  });

  it("applies custom className", () => {
    const { container } = render(
      <VizDataTable data={sampleData} className="custom-class" />
    );
