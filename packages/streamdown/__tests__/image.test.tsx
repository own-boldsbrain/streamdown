...
describe("Image Component", () => {
  it("should render an image with default wrapper", () => {
    ...
    const { container } = render(<ImageComponent src={src} alt={alt} />);
  ...
  it("should render an image with inline wrapper when in paragraph context", () => {
    ...
    const { container } = render(<ImageComponent src={src} alt={alt} node={node as any} />);
    
    const img = screen.getByAltText(alt);
    expect(img).toHaveAttribute("src", src);
    const wrapper = container.querySelector('[data-streamdown="image-inline-wrapper"]');
    expect(wrapper).toBeTruthy();
  });
  ...
  it("should include download button in both block and inline versions", () => {
    ...
    const { rerender, container } = render(
      <ImageComponent src="https://example.com/image.png" alt="Block image" />
    );
