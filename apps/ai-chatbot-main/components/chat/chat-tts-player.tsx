...
import { Play, Pause, SkipBack, SkipForward, Volume2, VolumeX, Mic, Settings, Download } from "lucide-react";
...
const ChatTTSPlayer: React.FC<TTSPlayerProps> = ({
  ...
  useEffect(() => {
    ...
    for (const word of words) {
      ...
      const wordDuration = Math.max(WORD_MIN_DURATION_SEC, word.length * WORD_DURATION_PER_CHAR);
            ...
            <span
              className={cn(
                ...
                i === currentWordIndex && isPlaying 
                  ? "bg-primary/20 font-medium px-0.5 rounded text-primary-foreground"
                  : ""
              )}
            >
              {item.word}
  ...
  return (
    <Card className={cn("overflow-hidden", className)}>
      <CardContent className={cn(
          ...
          <div className="flex items-center space-x-1">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                <DropdownMenuItem 
                  onClick={() => handlePlaybackRateChange(PLAYBACK_RATES.VERY_SLOW)}
                  className={cn(playbackRate === PLAYBACK_RATES.VERY_SLOW && "bg-primary/10")}
                >
                  Velocidade 0.5x
                </DropdownMenuItem>
                <DropdownMenuItem 
                  onClick={() => handlePlaybackRateChange(PLAYBACK_RATES.SLOW)}
                  className={cn(playbackRate === PLAYBACK_RATES.SLOW && "bg-primary/10")}
                >
                  Velocidade 0.75x
                </DropdownMenuItem>
                <DropdownMenuItem 
                  onClick={() => handlePlaybackRateChange(PLAYBACK_RATES.NORMAL)}
                  className={cn(playbackRate === PLAYBACK_RATES.NORMAL && "bg-primary/10")}
                >
                  Velocidade normal
                </DropdownMenuItem>
                <DropdownMenuItem 
                  onClick={() => handlePlaybackRateChange(PLAYBACK_RATES.FAST)}
                  className={cn(playbackRate === PLAYBACK_RATES.FAST && "bg-primary/10")}
                >
                  Velocidade 1.25x
                </DropdownMenuItem>
                <DropdownMenuItem 
                  onClick={() => handlePlaybackRateChange(PLAYBACK_RATES.FASTER)}
                  className={cn(playbackRate === PLAYBACK_RATES.FASTER && "bg-primary/10")}
                >
                  Velocidade 1.5x
                </DropdownMenuItem>
                <DropdownMenuItem 
                  onClick={() => handlePlaybackRateChange(PLAYBACK_RATES.VERY_FAST)}
                  className={cn(playbackRate === PLAYBACK_RATES.VERY_FAST && "bg-primary/10")}
                >
                  Velocidade 2x
                </DropdownMenuItem>
              </DropdownMenuContent>
        ...
        {/* Texto com destaque */}
        <div className={cn(
          "bg-muted/40 max-h-40 overflow-y-auto p-3 rounded",
          compact && "max-h-20"
        )}>
        ...
        <div className="flex flex-col space-y-2">
          {/* Barra de progresso */}
          <div className="flex items-center space-x-2">
            <span className="text-right text-xs text-muted-foreground w-8">
