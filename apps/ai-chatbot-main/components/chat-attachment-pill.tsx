...
function PureChatAttachmentPill({ attachment, isUploading = false, onRemove, className }: ChatAttachmentPillProps) {
  const { name, url, contentType } = attachment;
  const isImage = Boolean(contentType?.startsWith("image"));

  return (
    <div className={cn("group relative flex items-center gap-2 rounded-full border bg-muted px-2 py-1", className)}>
      <div className="flex h-6 w-6 items-center justify-center overflow-hidden rounded-full bg-background">
        {isImage && url ? (
          ...
          contentType?.startsWith("image") ? <ImageIcon size={12} /> : <FileIcon size={12} />
        )}
      </div>
