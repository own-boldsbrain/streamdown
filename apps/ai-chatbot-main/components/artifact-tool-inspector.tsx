...
const COST_PER_TOKEN_INPUT = 0.000_15; // USD per token
...
export const ArtifactToolInspector = memo<ArtifactToolInspectorProps>(
  ({
    ...
    const successRate =
      totalCalls > 0
        ? (successfulCalls / totalCalls) * PERCENTAGE_MULTIPLIER
        : 0;
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div className="space-y-1">
                  ...
                  <div className="flex items-center space-x-2">
                    <motion.p
                      ...
                      key={
                        animatedMetrics[
                          metric.name.replace(/\s+/g, "").toLowerCase()
                        ] || metric.value
                      }
                ...
                {metric.threshold && (
                  <div className="text-right">
                    ...
                    >
                      Meta:{" "}
                      {metric.name === "Custo Total"
                        ? formatCurrency(metric.threshold)
              ...
              {metric.threshold && (
                <Progress
                  className="mt-3 h-2"
                  value={Math.min(
                    (metric.value / metric.threshold) * PERCENTAGE_MULTIPLIER,
                    PROGRESS_MAX_VALUE
                  )}
                />
              )}
...
export const estimateCost = (
  ...
  return (
    inputTokens * COST_PER_TOKEN_INPUT + outputTokens * COST_PER_TOKEN_OUTPUT
  );
};
