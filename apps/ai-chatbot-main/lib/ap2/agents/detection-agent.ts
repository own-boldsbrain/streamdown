...
const detectAnomaly = async (input: any) => {
  ...
  const anomalies = consumption_history
    ...
    .map((current, index) => {
      ...
      if (
        Math.abs(change) > (validation_params?.anomaly_limits?.spike_pct || 30)
      ) {
        return {
  ...
  const risk_level =
    total_anomalies > 2 ? "high" : total_anomalies > 0 ? "moderate" : "low";
...
const calculateRiskScore = async (input: any) => {
  ...
  const overall_score = Object.values(breakdown).reduce(
    (sum, score) => sum + score,
    0
  );
...
const recognizePatterns = async (input: any) => {
  ...
  if (months >= 12) {
    ...
    if (Math.abs(summerConsumption - winterConsumption) > 100) {
      patterns.push({
        ...
        description:
          "Variação sazonal de consumo detectada entre verão e inverno",
        confidence: 0.75,
      });
    }
  }
  ...
  if (months >= 6) {
    const first3 = consumption_history
      .slice(0, 3)
      .reduce((sum, item) => sum + item.kwh, 0);
    const last3 = consumption_history
      .slice(-3)
      .reduce((sum, item) => sum + item.kwh, 0);
    ...
    if (Math.abs(change) > 15) {
      patterns.push({
        pattern_type:
          change > 0 ? "growing_consumption" : "reducing_consumption",
...
const validateCompliance = async (input: any) => {
  ...
  const avg_consumption =
    consumption_history.reduce((sum, item) => sum + item.kwh, 0) /
    consumption_history.length;
...
export const detectionAgent: AgentConfig = {
  slug: "detection",
  description:
    "Detecção de anomalias de consumo, cálculo de risk_score e validação de compliance (Lei 14.300)",
  inputSchema,
  outputSchema,
